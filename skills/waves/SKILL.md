...
2. Before dispatching, append a WIP dispatch line to memory-progress.md (continue skill
   ...
   model=<id> — next: review on return`. Both are effective values: the tier and model
   the dispatch actually ran at, never the ones the config asked for.
...
- Task → tier: briefs marked `mechanical: true` → mechanical; implementation and per-task
  ...
  `**Model tier:** <tier>` line, when present, overrides this.
- Tier → model: rows in `docs/agents/galdr.md` §Models (setup-written; row syntax and
  defaults named in setup, not here — models change, skill text shouldn't).
- Ladder: `mechanical → standard → top`, three attempts per task at any tier. First
  failure: retry. Second: escalate one tier and retry once, or retry at the same tier
  when already at `top`, which has nothing above it. Third: stop the wave (STOP condition
  below). Every runtime reads this ladder the same way. `EV [waves] <task-id>
  attempt=<n> <tier> → failed: <reason>`; `EV [waves] <task-id> attempt=2 escalate
  <old> → <new>`.
